...
plotter = VisdomLinePlotter(env_name='TCN')
...
def train(config):
    ...
    # load datasets
    train_dataset = dataset_module.get_dataset(config, transforms=transforms_train, part='test')
    val_dataset = dataset_module.get_dataset(config, transforms=transforms_val, part='val')

    train_dataloader = DataLoader(train_dataset, num_workers=config.train.num_workers,
                batch_size=config.train.batch_size, collate_fn=no_pad_collate,pin_memory=True)

    val_dataloader = DataLoader(val_dataset, num_workers=config.train.num_workers,
                batch_size=config.train.batch_size, collate_fn=no_pad_collate,pin_memory=True)
    ...
    model = TCN(input_size=64, num_channels=[64, 128, 256, 512, 1024], kernel_size=3, dropout=0.2, vocab_size=80)
    ...
    for epoch_idx in tqdm(range(config.train.get('epochs', 10))):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            output_length = torch.ceil(batch['input_lengths'].float()).int()
            ...
            optimizer.step()
            lr_scheduler.step()
            print(f'train_loss:{loss.item()}; duration:{time.time()-t}')
            loss_total += loss.item()
            #plotter.plot('Loss', 'Train Loss', f'Training Loss on batch at epoch {epoch_idx}', batch_idx, loss.item())
        ...
        plotter.plot('WER Train', 'Wer Train%', 'WER Train per epochs', epoch_idx, val_stats['val_loss'] * 100)
        plotter.plot('CER Train', 'Cer Train%', 'CER Train per epochs', epoch_idx,  val_stats['cer'] * 100)
        plotter.plot('loss', 'Train Loss', 'Loss per epochs', epoch_idx,
                  loss_total / len(train_dataloader))
        plotter.plot('Time', 'Time', 'Time per epochs', epoch_idx, time.time() - t2)
        ...
        #         val_stats['wer'].append(wer)
        #         val_stats['cer'].append(cer)
        #         #print(f'val_loss:{loss}; val_wer:{wer}; val_cer:{cer}')
        # for k, v in val_stats.items():
        #         val_stats[k] = np.mean(v)
        ...
        #         )
        # plotter.plot('WER', 'Wer %', 'WER per epochs', epoch_idx, prev_wer*100)
        # plotter.plot('CER', 'Cer %', 'CER per epochs', epoch_idx, val_stats['cer'] * 100)
        # plotter.plot('Loss2', 'Val Loss', 'Loss validation per epochs', epoch_idx, val_loss/len(val_dataloader))
        #         # wandb.save(os.path.join(config.train.get('checkpoint_path', 'checkpoints'), f'model_{epoch_idx}_{prev_wer}.pth'))
        #
        # print(f'epoch:{epoch_idx}; duration:{time.time() - t2}')
        # plotter.plot('TimeVal', 'TimeVal', 'Time validare per epochs', epoch_idx, time.time() - t3)
